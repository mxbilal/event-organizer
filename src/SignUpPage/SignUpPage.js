...
import { AccountConfirm, AccountVerify } from "./action";
...
export default function SignUpPage() {
  ...
  const AccountVerifyAPI = async () => {
    const response = await AccountVerify(uId, token);
    response !== undefined
  ...
  const AccountConfirmAPI = async () => {
    setLoading(true);
    const response = await AccountConfirm(uId, token, password, email);
    if (response.status === 200) {
      setScreen("signUp1");
    } else {
      setError(true);
      setErrorContent("An error occured, please try again later!");
    }
    setLoading(false);
  };
          ...
          {screen === "signUp" ? (
            <form onSubmit={handleSubmit}>
              <div className="sign-up-form">
                <p className="welcome-msg">Register</p>
                <p className="desc">Enter following details to register</p>
            ...
            <>
              <div className="sign-up-form-success">
                <svg
                  className="done-svg"
                ...
                <p className="welcome-msg">Registration Successful</p>
                <p className="desc">
